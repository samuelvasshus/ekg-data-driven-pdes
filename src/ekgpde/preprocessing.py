...
def gaussian_smooth(
    ...
    Args:
        ...
        half_width (int): Number of neighbours on each side included in the
            kernel (kernel length 2*half_width + 1). About 3*sigma covers
            almost all of the Gaussian.

    Returns:
        npt.NDArray[np.float64]: Smoothed signal, same length as `time_series`.
            Boundaries are handled by repeating the edge values, so the first
            and last `half_width` samples are less reliable.
    """
